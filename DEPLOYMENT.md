...
| GA4 measurement ID | `G-9Y0PEJY0XG` (in `src/components/CookieConsent.astro`) | Site-wide, but injected **only after the visitor consents to analytics** — not from `BaseLayout.astro`. Expect GA4 sessions to undercount vs. GSC clicks. |
| Brand `alternateName` | `src/layouts/BaseLayout.astro` — both JSON-LD nodes | Name variants (`Photo and Moto`, `PhotoandMoto`, `Photo&Moto`, lowercase forms). No `sameAs` yet — no social profiles exist. |
...
### Brand-name query behaviour

`photo&moto` and `photoandmoto` are read by Google as navigational brand
queries and return the site first. The spaced-out `photo and moto` is read as a
generic informational phrase about motorcycle photography — the AI Overview
defines the concept and lists competitors alongside us — so the site does not
rank first there.

`alternateName` in the JSON-LD (added 2026-08) is the schema-side hint towards
resolving this. It is not visible anywhere in GSC — it produces no rich result,
no Enhancements entry, no validation status — so the only way to judge it is to
run the query and watch the position over weeks. Nothing needs submitting; a
URL Inspection → **Request indexing** on `/fi/` and `/en/` just brings the
recrawl forward.

The real bottleneck is external: brand-entity recognition is driven by
mentions and inbound links using the name. Adding a `sameAs` array once
official social profiles exist will do more than any schema tweak.
