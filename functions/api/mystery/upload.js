// POST /api/mystery/upload — admin-only photo upload, stores base64 in D1
  ...
  try {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO photos (filename, content_type, image_data, uploader_name, year_estimate, people, location_notes, notes)
       VALUES (?, ?, ?, 'Admin', ?, ?, ?, ?)`
    ).bind(file.name, file.type, base64, yearEstimate, people, locationNotes, notes).run();

    return new Response(JSON.stringify({ success: true, id: result.meta.last_row_id, message: 'Kuva ladattu' }), { headers: corsHeaders });
  } catch (err) {
