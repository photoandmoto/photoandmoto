export async function onRequestPost(context) {
  ...
  try {
    ...
    const r = await env.DB.prepare(
      `INSERT INTO photos (filename,content_type,image_data,year_estimate,people,location_notes,notes) VALUES (?,?,?,?,?,?,?)`
    ).bind(file.name, file.type, btoa(bin), fd.get('year_estimate')||'', fd.get('people')||'', fd.get('location_notes')||'', fd.get('notes')||'').run();
